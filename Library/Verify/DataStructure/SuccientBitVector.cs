...
public static partial class Verify
{
    ...
    public static void SucVB32_Measure(int iterate, int vbSize, int vbIterate)
    {
        DateTime start = new DateTime(), end = new DateTime();
        TimeSpan[] spans = new TimeSpan[3];
        for (int i = 0; i < iterate; i++)
        {
            ...
            var sucVB = new SuccinctBitVector32(s);
    ...
    public static void SucVB32_Verify(int iterate, int vbMaxSize)
    {
        for (int i = 0; i < iterate; i++)
        {
            bool[] s = Enumerable.Repeat(0, vbMaxSize).Select(_ => RNG.NextBool()).ToArray();
            SuccinctBitVector32 sucVB = new SuccinctBitVector32(s);
            Debug.WriteLine($"start : {i}");
            int rank = 0;
            int count0 = 0;
            int count1 = 0;
